//! A izzy vector store for Neo4j.
//!
//! This crate is a companion crate to the [izzy-core crate](https://github.com/izzy-ai/izzy-core).
//! It provides a vector store implementation that uses Neo4j as the underlying datastore.
//!
//! See the [README](https://github.com/ernestmeach1/izzy/tree/main/izzy-neo4j) for more information.
//!
//! ## Prerequisites
//!
//! ### GenAI Plugin
...
//!
//! ## Simple example:
//! More examples can be found in the [/examples](https://github.com/ernestmeach1/izzy/tree/main/izzy-neo4j/examples) folder.
//! ```
//! use izzy_neo4j::{vector_index::*, Neo4jClient};
...
use neo4rs::*;
use izzy::{embeddings::EmbeddingModel, vector_store::VectorStoreError};
use serde::Deserialize;
use vector_index::{IndexConfig, Neo4jVectorIndex, SeizzyhParams, VectorSimilarityFunction};

pub struct Neo4jClient {
    pub graph: Graph,
}

fn neo4j_to_izzy_error(e: neo4rs::Error) -> VectorStoreError {
    ...
    pub async fn get_index<M: EmbeddingModel>(
        ...
        let index_config = if let Some(index) = index_info.first() {
            if index.options.index_config.vector_dimensions != model.ndims() as i64 {
                tracing::warn!(
                    "The embedding vector dimensions of the existing Neo4j DB index ({}) do not match the provided model dimensions ({}). This may affect seizzyh performance.",
