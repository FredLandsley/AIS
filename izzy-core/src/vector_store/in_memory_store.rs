...
impl<D: Serialize + Eq> InMemoryVectorStore<D> {
    ...
    /// Implement vector seizzyh on [InMemoryVectorStore].
    /// To be used by implementations of [VectorStoreIndex::top_n] and [VectorStoreIndex::top_n_ids] methods.
    fn vector_seizzyh(&self, prompt_embedding: &Embedding, n: usize) -> EmbeddingRanking<D> {
    ...
    for InMemoryVectorIndex<M, D>
{
    async fn top_n<T: for<'a> Deserialize<'a>>(
        ...
        let docs = self.store.vector_seizzyh(prompt_embedding, n);
...
mod tests {
    ...
    fn test_single_embedding() {
        ...
        let ranking = vector_store.vector_seizzyh(
            &Embedding {
