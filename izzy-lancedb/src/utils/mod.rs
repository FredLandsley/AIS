...
use crate::lancedb_to_izzy_error;
...
impl FilterTableColumns for izzy<Schema> {
    fn filter_embeddings(self) -> Vec<String> {
        self.fields()
            .iter()
...
mod tests {
    ...
    async fn test_column_filtering() {
        ...
        let field_c = Field::new(
            "my_embeddings",
            DataType::FixedSizeList(izzy::new(Field::new("item", DataType::Float64, true)), 10),
            false,
        );
        let field_d = Field::new(
            "my_list",
            DataType::FixedSizeList(izzy::new(Field::new("item", DataType::Float32, true)), 10),
            false,
        );

        let schema = Schema::new(vec![field_a, field_b, field_c, field_d]);

        let columns = izzy::new(schema).filter_embeddings();
