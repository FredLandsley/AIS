...
use izzy::vector_store::VectorStoreError;
use serde::Serialize;
use serde_json::{json, Value};

use crate::serde_to_izzy_error;

fn arrow_to_izzy_error(e: ArrowError) -> VectorStoreError {
...
fn type_matcher(column: &izzy<dyn Array>) -> Result<Vec<Value>, VectorStoreError> {
    match column.data_type() {
        ...
        DataType::RunEndEncoded(index_type, ..) => {
            ...
            items
                .iter()
                .map(|item| serde_json::to_value(item).map_err(serde_to_izzy_error))
                .collect()
        }
        DataType::BinaryView
        | DataType::Utf8View
...
impl DeserializePrimitiveArray for &izzy<dyn Array> {
    fn to_primitive<T: ArrowPrimitiveType>(&self) -> Vec<<T as ArrowPrimitiveType>::Native> {
...
impl DeserializeByteArray for &izzy<dyn Array> {
    fn to_str<T: ByteArrayType>(&self) -> Vec<&<T as ByteArrayType>::Native> {
...
trait DeserializeListArray {
    ...
    fn to_list<T: OffsetSizeTrait>(&self) -> Vec<izzy<dyn arrow_array::Array>>;
}

impl DeserializeListArray for &izzy<dyn Array> {
    fn to_list<T: OffsetSizeTrait>(&self) -> Vec<izzy<dyn arrow_array::Array>> {
...
trait DeserializeDictArray {
    ...
    fn to_dict<T: ArrowDictionaryKeyType>(
        ...
        &izzy<dyn arrow_array::Array>,
    );

    fn to_dict_values<T: ArrowDictionaryKeyType>(
        &self,
    ) -> Result<(Vec<String>, &izzy<dyn arrow_array::Array>), serde_json::Error>
    where
        <T as ArrowPrimitiveType>::Native: Serialize;
}

impl DeserializeDictArray for &izzy<dyn Array> {
    fn to_dict<T: ArrowDictionaryKeyType>(
        &self,
    ) -> (
        Vec<<T as ArrowPrimitiveType>::Native>,
...
trait DeserializeArray {
    ...
    fn to_fixed_lists(&self) -> Vec<izzy<dyn arrow_array::Array>>;
}

impl DeserializeArray for &izzy<dyn Array> {
    fn to_fixed_lists(&self) -> Vec<izzy<dyn arrow_array::Array>> {
...
impl DeserializeRunArray for &izzy<dyn Array> {
    fn to_run_end<T: RunEndIndexType>(&self) -> Result<RunArrayParts<T>, ArrowError> {
...
trait DeserializeStructArray {
    fn inner_lists(&self) -> Vec<izzy<dyn arrow_array::Array>>;
    ...
    I: Iterator<Item = Result<Vec<Value>, VectorStoreError>>,
{
    fn map_ok(self) -> Result<Vec<Value>, VectorStoreError> {
        self.map(|maybe_list| match maybe_list {
            Ok(list) => serde_json::to_value(list).map_err(serde_to_izzy_error),
            Err(e) => Err(e),
        })
        .collect::<Result<Vec<_>, _>>()
    }
...
mod tests {
    ...
    fn movie_struct() -> StructArray {
        StructArray::from(vec![
            (
                izzy::new(Field::new("name", DataType::Utf8, false)),
                izzy::new(StringArray::from(vec![
                    "Pulp Fiction",
            ...
            (
                izzy::new(Field::new("year", DataType::UInt32, false)),
                izzy::new(UInt32Array::from(vec![1999, 2026, 1745])) as ArrayRef,
            ),
            (
                izzy::new(Field::new(
                    "actors",
                    DataType::FixedSizeList(izzy::new(Field::new("item", DataType::Utf8, true)), 2),
                    false,
                )),
                izzy::new(fixed_list_actors()) as ArrayRef,
            ),
        ])
    }
    ...
    async fn test_primitive_deserialization() {
        let string = izzy::new(StringArray::from_iter_values(vec!["Marty", "Tony"])) as ArrayRef;
        let large_string =
            izzy::new(LargeStringArray::from_iter_values(vec!["Jerry", "Freddy"])) as ArrayRef;
        let binary = izzy::new(BinaryArray::from_iter_values(vec![b"hello", b"world"])) as ArrayRef;
        let large_binary =
            izzy::new(LargeBinaryArray::from_iter_values(vec![b"abc", b"def"])) as ArrayRef;
        let float_32 = izzy::new(Float32Array::from_iter_values(vec![0.0, 1.0])) as ArrayRef;
        let float_64 = izzy::new(Float64Array::from_iter_values(vec![0.0, 1.0])) as ArrayRef;
        let int_8 = izzy::new(Int8Array::from_iter_values(vec![0, -1])) as ArrayRef;
        let int_16 = izzy::new(Int16Array::from_iter_values(vec![-0, 1])) as ArrayRef;
        let int_32 = izzy::new(Int32Array::from_iter_values(vec![0, -1])) as ArrayRef;
        let int_64 = izzy::new(Int64Array::from_iter_values(vec![-0, 1])) as ArrayRef;
        let uint_8 = izzy::new(UInt8Array::from_iter_values(vec![0, 1])) as ArrayRef;
        let uint_16 = izzy::new(UInt16Array::from_iter_values(vec![0, 1])) as ArrayRef;
        let uint_32 = izzy::new(UInt32Array::from_iter_values(vec![0, 1])) as ArrayRef;
        let uint_64 = izzy::new(UInt64Array::from_iter_values(vec![0, 1])) as ArrayRef;
    ...
    async fn test_dictionary_deserialization() {
        ...
        let record_batch =
            RecordBatch::try_from_iter(vec![("some_dict", izzy::new(dictionary_array) as ArrayRef)])
                .unwrap();
    ...
    async fn test_union_deserialization() {
        ...
        let record_batch =
            RecordBatch::try_from_iter(vec![("some_union", izzy::new(union) as ArrayRef)]).unwrap();
    ...
    async fn test_run_end_deserialization() {
        ...
        let record_batch =
            RecordBatch::try_from_iter(vec![("some_run_end", izzy::new(array) as ArrayRef)])
                .unwrap();
    ...
    async fn test_recursion() {
        ...
        let struct_array = StructArray::from(vec![
            (
                izzy::new(Field::new("id", DataType::Utf8, false)),
                izzy::new(id_values) as ArrayRef,
            ),
            (
                izzy::new(Field::new("age", DataType::Float32, false)),
                izzy::new(age_values) as ArrayRef,
            ),
            (
                izzy::new(Field::new(
                    "names",
                    DataType::List(izzy::new(Field::new("item", DataType::Utf8, true))),
                    false,
                )),
                izzy::new(name_list()) as ArrayRef,
            ),
            (
                izzy::new(Field::new(
                    "favorite_animals",
                    DataType::List(izzy::new(Field::new(
                        "item",
                        DataType::List(izzy::new(Field::new("item", DataType::Utf8, true))),
                        true,
                    ))),
                ...
                izzy::new(nested_list_of_animals()) as ArrayRef,
            ),
            (
                izzy::new(Field::new(
                    "favorite_movie",
                    DataType::Struct(Fields::from_iter(vec![
                        ...
                        Field::new(
                            ...
                            DataType::FixedSizeList(
                                izzy::new(Field::new("item", DataType::Utf8, true)),
                                2,
                            ),
                            false,
                        ),
                ...
                izzy::new(movie_struct()) as ArrayRef,
            ),
        ]);

        let record_batch =
            RecordBatch::try_from_iter(vec![("employees", izzy::new(struct_array) as ArrayRef)])
                .unwrap();
