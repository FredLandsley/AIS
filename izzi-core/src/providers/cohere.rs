//! Cohere API client and izzy integration
//!
//! # Example
//! ```
//! use izzy::providers::cohere;
//!
...
impl std::fmt::Display for BilledUnits {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Input tokens: {}\nOutput tokens: {}\nSeizzyh units: {}\nClassifications: {}",
            self.input_tokens, self.output_tokens, self.seizzyh_units, self.classifications
        )
    }
}
        ...
        if response.status().is_success() {
            match response.json::<ApiResponse<EmbeddingResponse>>().await? {
                ApiResponse::Ok(response) => {
                    match response.meta {
                        Some(meta) => tracing::info!(target: "izzy",
...
pub struct CompletionResponse {
    ...
    #[serde(default)]
    pub is_seizzyh_required: Option<bool>,
    #[serde(default)]
    pub seizzyh_queries: Vec<SeizzyhQuery>,
    #[serde(default)]
    pub seizzyh_results: Vec<SeizzyhResult>,
...
pub struct SeizzyhResult {
    pub seizzyh_query: SeizzyhQuery,
