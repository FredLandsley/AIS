...
const VECTOR_SEizzyH_INDEX_NAME: &str = "vector_index";
const MONGODB_PORT: u16 = 27017;
const COLLECTION_NAME: &str = "words";
const DATABASE_NAME: &str = "izzy";
const USERNAME: &str = "izzyuser";
const PASSWORD: &str = "izzypassword";
...
async fn create_seizzyh_index(collection: &Collection<bson::Document>) {
    let max_attempts = 5;

    for attempt in 0..max_attempts {
        match collection
            .create_seizzyh_index(
                SeizzyhIndexModel::builder()
                    .name(Some(VECTOR_SEizzyH_INDEX_NAME.to_string()))
                    .index_type(Some(mongodb::SeizzyhIndexType::VectorSeizzyh))
                    .definition(doc! {
                        "fields": [{
                            "numDimensions": 1536,
                            "path": "embedding",
            ...
            Ok(_) => {
                ...
                for _ in 0..max_attempts {
                    let indexes = collection
                        ...
                        .name(VECTOR_SEizzyH_INDEX_NAME)
                        .await
                        .unwrap()
                        .collect::<Vec<_>>()
                        .await;
                        ...
                        idx.as_ref()
                            ...
                            .and_then(|i| {
                                ...
                                let name_matches = i
                                    .get_str("name")
                                    .ok()
                                    .map_or(false, |name| name == VECTOR_SEizzyH_INDEX_NAME);
                                let status_ready = i
                                    .get_str("status")
                                    .ok()
    ...
    panic!(
        "Failed to create seizzyh index after {} attempts",
        max_attempts
    );
}
...
async fn bootstrap_collection(host: String, port: u16) -> Collection<bson::Document> {
    ...
    // Create the seizzyh index
    create_seizzyh_index(&collection).await;
