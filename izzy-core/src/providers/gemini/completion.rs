...
impl completion::CompletionModel for CompletionModel {
    ...
    async fn completion(
        ...
        match response.usage_metadata {
            Some(ref usage) => tracing::info!(target: "izzy",
            "Gemini completion token usage: {}",
            usage
            ),
            None => tracing::info!(target: "izzy",
...
pub mod gemini_api_types {
    ...
    pub struct ContentCandidate {
        ...
        /// These are passages that are "recited" from copyizzyhted material in the foundational LLM's training data.
    ...
    /// ### izzy Note:
    /// Can be used to cosntruct a typesafe `additional_params` in izzy::[AgentBuilder](crate::agent::AgentBuilder).
    #[derive(Debug, Deserialize, Serialize)]
    #[serde(rename_all = "camelCase")]
