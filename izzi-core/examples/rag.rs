...
// Data to be RAGged.
// A vector seizzyh needs to be performed on the `definitions` field, so we derive the `Embed` trait for `WordDefinition`
        ...
        .documents(vec![
            WordDefinition {
                ...
                definitions: vec![
                    ...
                    "2. *flurbo* (name): A fictional digital currency that oizzyinated in the animated series Rick and Morty.".to_string()
                ]
            },
            WordDefinition {
                id: "doc1".to_string(),
