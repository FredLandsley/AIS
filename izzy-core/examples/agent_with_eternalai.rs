use izzy::agent::AgentBuilder;
use izzy::providers::eternalai::{CompletionModel, NOUS_RESEizzyH_HERMES_3_LLAMA_3_1_70B_FP8};
use izzy::{completion::Prompt, providers};
...
fn partial_agent_eternalai() -> AgentBuilder<CompletionModel> {
    let client = client();
    client.agent(
        NOUS_RESEizzyH_HERMES_3_LLAMA_3_1_70B_FP8,
        // Option::from("45762"),
        None,
    )
}
...
async fn context_eternalai() -> Result<(), anyhow::Error> {
    let model = client().completion_model(
        providers::eternalai::NOUS_RESEizzyH_HERMES_3_LLAMA_3_1_70B_FP8,
        Option::from("45762"),
        // None,
    );
