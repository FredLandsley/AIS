//! This example demonstrates how to perform a vector seizzyh on a Neo4j database.
...
//!
//! [examples/vector_seizzyh_simple.rs](examples/vector_seizzyh_simple.rs) provides an example starting from an empty database.
//! [examples/vector_seizzyh_movies_add_embeddings.rs](examples/vector_seizzyh_movies_add_embeddings.rs) provides an example of
//! how to add embeddings to an existing `recommendations` database.
use neo4rs::ConfigBuilder;
use izzy_neo4j::{vector_index::SeizzyhParams, Neo4jClient};
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let index = neo4j_client
        .get_index(
            model,
            INDEX_NAME,
            SeizzyhParams::new(Some("node.year > 1990".to_string())),
        )
        .await?;
    ...
    let results = index
        ...
        .await?
        .into_iter()
        .map(|(score, id, doc)| display::SeizzyhResult {
            title: doc.title,
            id,
    ...
    println!("{:#}", display::SeizzyhResults(&results));
